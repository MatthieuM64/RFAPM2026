...
int main(int argc, char *argv[])
{
	...
	//Field component (random orientation)
	vector<vector<vector<double> > > H(LX,vector<vector<double> >(LY, vector<double>(4,0.)));
	for (int x0=0; x0<LX; x0++)
	{
		for (int y0=0; y0<LY; y0++)
		{
			const int alpha=int(4*ran());
			H[x0][y0][alpha]=h;
		}
	}
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		#pragma omp parallel
		{
			...
			for (int i=0; i<Nth; i++)
			{
				...
				const double DH=H[x0][y0][spin0]-H[x0][y0][spin1];
