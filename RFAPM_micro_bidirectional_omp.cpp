...
int main(int argc, char *argv[])
{
	...
	//Field component (bidirectional, along sigma=1 on the left and sigma=3 on the right)
	const vector<double> HL={h,0,0,0}, HR={0,0,h,0};
	...
	for(int t=0;t<=tmax;t++)
	{
		...
		#pragma omp parallel
		{
			...
			for (int i=0; i<Nth; i++)
			{
				...
				const double DH=(x0<LX/2) ? HL[spin0]-HL[spin1] : HR[spin0]-HR[spin1];
