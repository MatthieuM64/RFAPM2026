...
/*HYDRODYNAMIC THEORY OF ACTIVE POTTS MODEL UNDER A RANDOM ORIENTATIONAL FIELD*/
...
{
	double magl=0.8, dr=0.25, dH=0.01;
	double phi=(rho0-rhog)/(rhol-rhog);
	if (init==1) //Disordered state.
	{
		magl=0;
	}
	if (init==2) //Ordered state.
	{
		dr=0.;
	}
	...
	for (int x=0; x<NX; x++)
	{
		for (int y=0; y<NY; y++)
		{
			...
			if (init<=2)
			{
				rho=rho0*(1+dr*sin(8*M_PI*y/NY)*sin(8*M_PI*x/NX));
				mag=magl;
			}
			else if (init==3 and abs(x-0.5*NX)<0.5*NX*phi-dH*NX*cos(2*M_PI*y/NY)) //vertical band.
			{
				rho=rhol;
				mag=magl;
			}
			else if (init==4 and abs(y-0.5*NY)<0.5*NY*phi-dH*NY*cos(2*M_PI*x/NX)) //horizontal band.
			{
				rho=rhol;
				mag=magl;
			}
			...
			RHO3[x][y]=0.25*rho*(1-mag);
			RHO4[x][y]=0.25*rho*(1-mag);
		}
	}
}
...
void finiteDiff(vector<vector<double> > &RHO1, vector<vector<double> > &RHO2, vector<vector<double> > &RHO3, vector<vector<double> > &RHO4, const double &Dpara, const double &Dperp, const double &v0, const double &gamma0, const double &beta, const vector<vector<double> > &HH, const double &r, const int &NX, const int &NY, const double &dx)
{
	...
	#pragma omp parallel default(shared)
	{
		...
		for (int x=0; x<NX; x++)
		{
			...
			for (int y=0; y<NY; y++)
			{
				...
				const int site=int(x*dx)*(dx*NY)+int(y*dx);
				const vector<double> HLOC=HH[site];
...
void exportDensityState(const vector< vector<double> > &RHO1, const vector< vector<double> > &RHO2, const vector< vector<double> > &RHO3, const vector< vector<double> > &RHO4, const double &beta, const double &epsilon, const double &rho0, const double &h, const int &LX, const int &LY, const int &NX, const int &NY, const int &init, const int &RAN, const double &t)
{
	const int a=NX/LX;
	...
	ssDensity << "./data_RFAPM_dynamics2d/RFAPM_density_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_h=" << h << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".bin";
	...
	ssState << "./data_RFAPM_dynamics2d/RFAPM_state_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_h=" << h << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".bin";
...
void exportProfilesAlongX(const vector< vector<double> > &RHO1, const vector< vector<double> > &RHO2, const vector< vector<double> > &RHO3, const vector< vector<double> > &RHO4, const double &beta, const double &epsilon, const double &rho0, const double &h, const int &LX, const int &LY, const int &NX, const int &NY, const int &init, const int &RAN, const double &t)
{
	...
	stringstream ss;
	ss << "./data_RFAPM_dynamics1d/RFAPM_profile_along_x_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_h=" << h << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
...
void exportProfilesAlongY(const vector< vector<double> > &RHO1, const vector< vector<double> > &RHO2, const vector< vector<double> > &RHO3, const vector< vector<double> > &RHO4, const double &beta, const double &epsilon, const double &rho0, const double &h, const int &LX, const int &LY, const int &NX, const int &NY, const int &init, const int &RAN, const double &t)
{
	...
	int returnSystem=system("mkdir -p data_RFAPM_dynamics1d/");
	stringstream ss;
	ss << "./data_RFAPM_dynamics1d/RFAPM_profile_along_y_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_h=" << h << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << "_t=" << t << ".txt";
	string nameProfile = ss.str();
...
void ReadCommandLine(int argc, char** argv, double &beta, double &epsilon, double &rho0, double &h, int &LX, int &LY, int &init, double &rhog, double &rhol, double &dx, double &dt, double &tmax, int &RAN, int &THREAD_NUM)
{
...
int main(int argc, char *argv[])
{
	...
	ReadCommandLine(argc,argv,beta,epsilon,rho0,h,LX,LY,init,rhog,rhol,dx,dt,tmax,RAN,THREAD_NUM);
	...
	vector<vector<double> > HH(LX*LY,vector<double>(4,0.));
	
	for (int x=0; x<LX; x++)
	{
		for (int y=0; y<LY; y++)
		{
			HH[x*LY+y][int(4*ran())]=h;
		}
	}
	...
	ssAverages << "./data_RFAPM_averages/RFAPM_averages_beta=" << beta << "_epsilon=" << epsilon << "_rho0=" << rho0 << "_h=" << h << "_LX=" << LX << "_LY=" << LY << "_init=" << init << "_ran=" << RAN << ".txt";
	...
	for(int t=0;t<=Nsteps;t++)
	{
		...
		{
			exportProfilesAlongX(RHO1,RHO2,RHO3,RHO4,beta,epsilon,rho0,h,LX,LY,NX,NY,init,RAN,t*dt);
			exportDensityState(RHO1,RHO2,RHO3,RHO4,beta,epsilon,rho0,h,LX,LY,NX,NY,init,RAN,t*dt);
		...
		finiteDiff(RHO1,RHO2,RHO3,RHO4,Dpara,Dperp,v0,gamma0,beta,HH,r,NX,NY,dx);
	}
	return 0;
}
