...
/*HYDRODYNAMIC THEORY OF ACTIVE POTTS MODEL UNDER A HOMOGENEOUS UNIDIRECTIONAL FIELD*/
...
void meshInit(vector<vector<double> > &RHO1, vector<vector<double> > &RHO2, vector<vector<double> > &RHO3, vector<vector<double> > &RHO4, const int &init, const double &rhog, const double &rhol, const double &rho0, const double &beta, const double &r, const int &NX, const int &NY)
{
	...
	const double phi=(rho0-rhog)/(rhol-rhog);
...
int main(int argc, char *argv[])
{
	...
	//Numerical parameters: init=initial condition, dt=discrete time, tmax=maximal time, dx=meshgrid size, rhog=min density, rhol=max density, THREAD_NUM=number of threads.
	...
	const vector<double> HH={h,0,0,0};
